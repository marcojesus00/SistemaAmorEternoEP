...
LEFT JOIN
    ...
    Public Function GetReceiptByServiceDataForGridview()
        ...
        Try
            ...
            Dim finalData = data1.Select(Function(s) New With {.Codigo = s.ServicioId, s.Servicio, s.Cantidad, s.Contratos, s.Prima, s.Cuota, .Valor = FormattingHelper.ToLempiras(s.Valor)}).ToList()
    ...
    Public Sub RouteOfReceiptsMap(keyValue As String)
        ...
        For Each receipt As VentasDto In receipts
            Dim tooltipMsg = $"<b>Documento:{receipt.Recibo.Trim}</b> <br>Cliente: {receipt.Cliente} <br>Cobrado: {FormattingHelper.ToLempiras(receipt.Por_lempira)} <br>Hora: {receipt.Hora} <br>Fecha: {receipt.Fecha:dd/MM/yyyy}"
